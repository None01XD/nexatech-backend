...
(function(){
  ...
  function createProductCard(p){
    ...
    // normalize imagen field: allow full URLs, absolute paths, or simple filenames
    let imgSrc = p.imagen || '';
    if(!imgSrc){
      ...
      imgSrc = '/images/' + imgSrc; // treat as filename placed in /images
    }
    img.src = imgSrc;
    img.onerror = () => { img.onerror = null; img.src = 'https://via.placeholder.com/800x600?text='+encodeURIComponent(p.nombre||'Producto'); };
    ...
    footer.appendChild(price); footer.appendChild(cta);
  ...
  function render(list){
    ...
    if(dotsWrap){
      cards.forEach((_,i)=>{
        const b = document.createElement('button'); b.type='button'; b.addEventListener('click',()=> goTo(i));
  ...
  function goTo(i){ index = ((i % cards.length) + cards.length) % cards.length; update(); }

  prev?.addEventListener('click', ()=> goTo(index-1));
  next?.addEventListener('click', ()=> goTo(index+1));

  // drag support on view
  let pointerStart = null, pointerDelta = 0;
  view?.addEventListener('pointerdown', (e)=>{ pointerStart = e.clientX; pointerDelta = 0; view.setPointerCapture(e.pointerId); });
  view?.addEventListener('pointermove', (e)=>{ if(pointerStart===null) return; pointerDelta = e.clientX - pointerStart; });
  view?.addEventListener('pointerup', ()=>{ if(pointerStart===null) return; if(Math.abs(pointerDelta)>40){ goTo(pointerDelta>0?index-1:index+1); } pointerStart=null; pointerDelta=0; });
  view?.addEventListener('pointercancel', ()=>{ pointerStart=null; pointerDelta=0; });

  window.addEventListener('resize', ()=> setTimeout(update,80));

  // blur toggle
  blurToggle?.addEventListener('change', ()=>{ track.classList.toggle('no-blur', !blurToggle.checked); });

  // Theme toggle removed — site uses single nocturnal palette

  // init
  loadProducts();
  // expose goTo for debug
